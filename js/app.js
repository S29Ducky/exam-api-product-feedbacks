...
async function searchElements() {
	const reponse = await fetch(`http://51.38.232.174:3002/v1/feedbacks/${1}`, {
		method: "GET",
	});
	const data = await reponse.json();
	const Allfeedback = document.querySelector(".feedback-wrapper");
	for (let i = 0; i < data.length; i++) {
        ...
        `;
		Allfeedback.appendChild(feedback);
	}
}
